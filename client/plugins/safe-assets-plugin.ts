...
export default function safeAssetsPlugin(config?: Partial<Config>): PluginOption {
  ...
  const generate = async () => {
    if (!publicDir) {
      console.error('Public directory not found, skipping safe assets generation');
      return;
    }
    ...
    const publicFiles = publicFilesFullPath.map((file) => file.replace(publicDir, ''));
      ...
      for (let i = 0; i < parts.length; i++) {
        ...
        if (i === parts.length - 1) {
          ...
          if (typeof current[part] === 'string') {
            throw new Error(`File ${path} conflicts with ${current[part]} at ${part}`);
          }
