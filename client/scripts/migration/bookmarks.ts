...
const parseOldBookmarks = (rawBookmarks: unknown, userId?: string) => {
  ...
  const migratedBookmarks = bookmarks
    .map((bookmark) => {
      ...
      const pageType = bookmark.favorite_data.type === 'player' ? 'player_stats' : 'clan_stats';
...
export const getOldUserBookmarks = async (userId: string, discordId: string) => {
...
`.catch((error) => {
  ...
  try {
    ...
    await migrationDb.insert(usersTable).values(tempUsers).onConflictDoNothing().execute();
