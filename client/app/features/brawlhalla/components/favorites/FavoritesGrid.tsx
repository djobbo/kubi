...
import type { Bookmark, NewBookmark } from '@/features/bookmarks/schema/bookmarks';
...
const BookmarkDisplay = ({ bookmark }: BookmarkDisplayProps) => {
  ...
  return (
    <div
      className={cn('relative rounded-lg hover:bg-secondary', bookmarkWithRemoveButtonClass)}
      key={`${bookmark.pageType}/${bookmark.id}`}
    >
      <Link {...bookmarkLinkData} className={cn('flex items-center gap-2 px-3 py-3')}>
        {icon}
