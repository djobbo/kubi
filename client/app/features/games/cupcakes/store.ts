...
import { BASE_ITEMS, type BaseItem, type BaseUpgrade, type ItemId } from './items';
...
export const useCupcakesStore = create<CupcakesStore>()((set) => ({
  ...
  onClick: (x = 0, y = 0) =>
    set((state) => {
      const clickOutput = state.items.reduce((acc, item) => acc + item.count * item.output, 1);
  ...
  buyItem: (id, amount) => {
    set((state) => {
      ...
      return {
        ...
        items: state.items.map((i) => (i.id === id ? { ...i, count: i.count + amount } : i)),
      };
    });
  },
        ...
        items: state.items.map((i) =>
          i.id === itemId
            ? {
                ...
                interval: i.interval ? i.interval * (upgrade.intervalMultiplier ?? 1) : i.interval,
              }
            : i
        ),
      };
  ...
  addFloatingCupcake: (value, x = 0, y = 0) => {
    ...
    set((state) => {
      ...
      return {
        floatingCupcakes: [...state.floatingCupcakes, { id, value, position: { x, y }, direction }],
      };
    });
