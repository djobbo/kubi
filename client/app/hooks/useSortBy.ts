...
export const useSortBy = <ElementType, Option extends string>(
  ...
  const [sortDirection, setSortDirection] = useState<SortDirection>(defaultDirection); // 1 = ascending, -1 = descending

  const sortedArray = useMemo(
    () => array.slice(0).sort((a, b) => compareFns[sortBy].sortFn(a, b) * sortDirection),
  ...
  return {
    ...
    sortBy,
    setSortBy,
    options: (Object.entries(compareFns) as [string, CompareFn<ElementType>][]).map(
      ([value, { label }]) => ({
        value,
        label,
      })
