...
export const Route = createFileRoute('/rankings/clans/$')({
  ...
  head: ({ loaderData }) => {
    ...
    return {
      meta: seo({
        title: t`Brawlhalla Clan Rankings - Page ${page} ${formatedSearch} • Corehalla`,
        description: t`Brawlhalla Clan Rankings - Page ${page} ${formatedSearch} • Corehalla`,
      }),
    };
  },
});
...
function RouteComponent() {
  ...
  const [search, setSearch, immediateSearch] = useDebouncedState(clan ?? '', 500);
        ...
        {rankings
          ...
          .map((clan, i) => {
            return (
              <div
                key={clan.id}
                className={cn('px-4 py-2 w-full h-full flex items-center gap-4 hover:bg-border', {
                  'bg-secondary': i % 2 === 0,
                })}
              >
                {showClanRank && (
                ...
                <div className="w-40 flex items-center justify-center">
                  {clan.createdAt ? clan.createdAt.toLocaleDateString() : t`N/A`}
                </div>
