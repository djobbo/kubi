import { Google, Discord, generateState } from "arctic";
import { eq, and, gt } from "drizzle-orm";
import { db } from "../db";
import { env } from "../env";
import { oauthAccountsTable, usersTable, sessionsTable, type NewUser, type NewOAuthAccount, type NewSession, GOOGLE_PROVIDER_ID, DISCORD_PROVIDER_ID, type Provider } from "@dair/schema/src/auth";
...
export async function validateOAuthCallback(
  ...
  const tokens =  provider === GOOGLE_PROVIDER_ID ?   await google.validateAuthorizationCode(code, env.AUTH_SECRET) : await discord.validateAuthorizationCode(code, null);
  ...
  const newUser: NewUser = {
    ...
    username: "username" in userInfo ? userInfo.username : userInfo.email.split("@")[0] || "anonymous",
    avatarUrl: "picture" in userInfo ? userInfo.picture : `https://cdn.discordapp.com/avatars/${userInfo.id}/${userInfo.avatar}.png`,
  };
...
export function createAuthorizationURL(provider: Provider) {
  ...
  return provider === GOOGLE_PROVIDER_ID ?
    google.createAuthorizationURL(state, env.AUTH_SECRET, ["https://www.googleapis.com/auth/userinfo.email", "https://www.googleapis.com/auth/userinfo.profile"]) :
