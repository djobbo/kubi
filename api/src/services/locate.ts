...
export const getRegion = async (ip: string) => {
  ...
  const closestServer = servers.reduce<{ server: (typeof servers)[number], distance: number }>((closest, server) => {
    const distance = getDistance(lat, lon, server.location.lat, server.location.lon)
    return distance < closest.distance ? { server, distance } : closest
  }, { server: servers[0], distance: Infinity })
