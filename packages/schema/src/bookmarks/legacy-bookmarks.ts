...
import { z } from 'zod';
import { pageTypeEnum, type Meta, metaSchema, pageTypeSchema } from './bookmarks';
...
export const legacyBookmarksTable = sqliteTable(
  ...
  (table) => [
    uniqueIndex('unique_legacy_bookmark').on(table.discordId, table.pageType, table.pageId),
  ]
);
...
export const legacyBookmarksInsertSchema = createInsertSchema(legacyBookmarksTable, {
