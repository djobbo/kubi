...
import { type ReplayFaces, parseFaces } from "./faces"
import { type ReplayGameData, parseGameData } from "./game-data"
import { type ReplayHeader, parseHeader } from "./header"
import { type ReplayInputList, parseInputList } from "./input-list"
import { type ReplayResult, parseResult } from "./result"
...
export const parseReplay = (data: ArrayBuffer) => {
	...
	while (!reachedReplayEnd) {
		...
		switch (replayObjectType) {
			case ReplayObjectTypeEnum.Header:
				if (header) throw new Error("Duplicate replay header")
				console.log("Header")
				header = parseHeader(bitReader)
				console.log(header)
				break
			case ReplayObjectTypeEnum.GameData:
				if (gameData) throw new Error("Duplicate game data")
	...
	const inputsWithNames = inputs
		? Object.fromEntries(
				Array.from(inputs.entries()).map(([key, inputs]) => [
					key,
					inputs.map(({ inputFlags, ...input }) => ({
						...input,
						inputFlags,
						inputs: getReplayInputFlagsNames(inputFlags),
					})),
				]),
			)
		: null
