	...
	return fullLegends.reduce<{
		...
		(acc, legend) => {
			...
			return {
				...
				damagedealt:
					acc.damagedealt + Number.parseInt(legend.stats.damagedealt),
				damagetaken:
					acc.damagetaken + Number.parseInt(legend.stats.damagetaken),
			}
		},
		{
			matchtime: 0,
		...
		(acc, legend) => ({
			unarmed: {
				...
				damageDealt:
					...
					Number.parseInt(legend.stats?.damageunarmed ?? "0"),
				matchtime:
			...
			gadgets: {
				...
				damageDealt:
					...
					Number.parseInt(legend.stats?.damagegadgets ?? "0"),
			},
			throws: {
				kos: acc.throws.kos + (legend.stats?.kothrownitem ?? 0),
				damageDealt:
					acc.throws.damageDealt +
					Number.parseInt(legend.stats?.damagethrownitem ?? "0"),
			},
		}),
		{
			unarmed: {
