...
async function getIpAndLocation(server: { id: RankedRegion; url: string }): Promise<ServerWithInfo | null> {
  try {
...
async function main() {
  ...
  await writeFile('src/constants/servers.ts', `export const servers = ${JSON.stringify(filteredResults, null, 2)} as const`)
