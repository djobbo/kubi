...
const syncLocalApiUrls = Effect.fnUntraced(function* () {
    const fs = yield* FileSystem.FileSystem
    const existing = yield* fs.readFileString(ENV_PATH)
    const appPort = getEnvVar(existing, "APP_PORT", "3000")
    const apiPort = getEnvVar(existing, "API_PORT", "3001")
...
const program = Effect.gen(function* () {
    ...
    const appPort = getEnvVar(envContent, "APP_PORT", "3000")
    yield* Effect.logInfo(`Supabase Studio: http://127.0.0.1:${STUDIO_PORT}`)
    yield* Effect.logInfo(
        `Web app: http://localhost:${appPort} (run vp run dev)`,
    )
})
