...
| `apps/api` typecheck                                    | **Done** | `vp run check:types` green                                                                                    |
| Context.Service migration                               | **Done** | All API services use `Context.Service` + `Layer.effect`                                                       |
| Zod removal                                             | **Done** | `@dair/schema` package removed; no `zod` imports in apps/packages                                             |
| Drizzle `defineRelations`                               | **Done** | Central [packages/db/src/relations.ts](../packages/db/src/relations.ts); per-table `_relations` removed       |
...
- [x] Convert each class to `Context.Service` + `make`
- [x] Replace `static readonly layer = this.Default.pipe(...)` with `Layer.effect(this, this.make).pipe(...)`
- [x] Grep `\.Default` under `apps/api` and update composition in [apps/api/src/index.ts](../apps/api/src/index.ts)
...
- [x] `rg 'Effect\.Service' apps/api` returns empty
- [x] `rg 'Context\.Tag' apps/api` returns empty (or only intentional legacy)
- [x] `vp run -F @dair/api check:types` — TS2551 `Service` / `Default` errors gone
...
- [x] Remove `zod` from [packages/brawlhalla-api/package.json](../packages/brawlhalla-api/package.json) (already no dep; delete imports)
- [x] `rg 'from "zod' packages/brawlhalla-api` empty
- [x] `vp run check:types` covers brawlhalla-api via dependents
...
- [x] Remove `zod` from migrator `package.json` if present
- [x] `vp run -F @dair/migrator check:types`

### Workers / client

- [x] `rg 'from "zod' apps packages` empty
...
- [x] Confirm zero imports: `rg '@dair/schema' --glob '*.{ts,tsx}'`
- [x] Removed `packages/schema` (`@dair/schema` SQLite-era package)
- [x] Update [03-supabase-to-pg.md](./03-supabase-to-pg.md) — mark SQLite `@dair/schema` as removed
...
- [x] Read Drizzle 1.0 relation docs / examples in `.repos/drizzle-orm` (refresh vendored if needed)
- [x] Add [packages/db/src/relations.ts](../packages/db/src/relations.ts) with `defineRelations(schema, (r) => ({ ... }))`
- [x] Remove per-table `export const fooRelations = relations(...)` blocks
- [x] Pass relational config into `PgDrizzle.make` via [apps/api/src/services/db/index.ts](../apps/api/src/services/db/index.ts)
- [x] `rg 'drizzle-orm/_relations' packages/db` empty
- [x] `vp run check:types` for workspaces that import `@dair/db`
...
- [ ] No `effect@3` or `@effect/platform` (non-node) in lockfile — verify after `vp install`
- [x] No `zod` / `drizzle-zod` in active package imports
- [x] `rg 'Effect\.Service|from "zod|@effect/platform"' apps packages` clean (except docs/comments in `.plans`)
- [ ] [02-effect-v4.md](./02-effect-v4.md) checkboxes updated; this doc sections checked off
- [x] AGENTS.md Effect row says v4 + `Context.Service` pattern
