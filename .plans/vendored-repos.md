...
| `.repos/vite-plus`       | [voidzero-dev/vite-plus](https://github.com/voidzero-dev/vite-plus)     | `main`   | **Toolchain:** `packageManager` / `vp` / pnpm version source — [07-vite-plus-toolchain.md](./07-vite-plus-toolchain.md) |
...
Sync logic lives in [scripts/setup/sync-vendored-repos.mts](../scripts/setup/sync-vendored-repos.mts) and is exported as `Effect.fn("syncVendoredRepos")`.
...
import { syncVendoredRepos } from './scripts/setup/sync-vendored-repos.mts'
...
1. Add an entry to `VENDORED_REPOS` in [scripts/setup/sync-vendored-repos.mts](../scripts/setup/sync-vendored-repos.mts) (`name`, `url`, `branch`).
